...
| Felan | built-in lifecycle reporter | state and session |
...
For agents with complete lifecycle hooks, the integration or built-in reporter is authoritative while actively reporting for the running pane. Herdr uses those reports for `idle`, `working`, `blocked`, and session identity. It does not also run screen manifest fallback for that same lifecycle authority. This avoids two competing sources of truth.
...
Install an integration for each agent that needs one to give Herdr hook or plugin reports instead of screen detection alone. Agents such as Felan that ship a built-in reporter need no separate installation:
