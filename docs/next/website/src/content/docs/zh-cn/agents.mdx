...
对于具备完整生命周期钩子的智能体,当集成或内置上报器在为运行中的窗格主动上报时,这些上报就是权威。Herdr 用这些上报来决定 `idle`、`working`、`blocked` 和会话身份。对同一个生命周期权威,它不会再并行运行屏幕清单兜底。这样可以避免出现两个相互竞争的事实来源。
...
为需要集成的智能体安装集成;它让 Herdr 获得钩子或插件上报,而不是只靠屏幕检测。像 Felan 这样内置上报器的智能体无需另行安装:
...
具有独立集成的智能体各有自己的集成名称和行为。按智能体的细节和完整安装列表见[集成](/zh-cn/docs/integrations/)。如果你正在构建智能体,[自定义集成指南](/zh-cn/docs/integrations/#集成你自己的智能体)介绍了如何在不为 Herdr 添加原生支持的情况下上报生命周期状态。
