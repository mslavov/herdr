...
Herdr only resumes panes that reported a native session reference through a current official Herdr integration or recognized built-in reporter.
...
Native session restore requires these Herdr integration versions or reporter support:

| Agent | Requirement | Resume command |
| --- | --- | --- |
| Pi | `2` | `pi --session <path-or-id>` |
| Felan | built-in reporter | `felan --session <id>` |
...
Run `herdr integration status` to check installed integration versions. Reinstall outdated integrations with `herdr integration install <agent>`; Felan needs no separate installation.

Felan session IDs are resolved from Felan's configured or default session directory. A session opened with a one-off `--session-dir` outside that directory cannot be restored automatically.
