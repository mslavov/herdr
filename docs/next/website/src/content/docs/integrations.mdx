...
| --- | --- | --- |
| Lifecycle authority | Felan, Pi, OMP, Kimi Code CLI, OpenCode, Kilo Code CLI, MastraCode | While actively reporting for the pane, built-in reporter, hook, or plugin events author `idle`, `working`, and `blocked`. Herdr does not also use screen manifest fallback for that same lifecycle authority. |
...
Felan ships its Herdr lifecycle reporter. There is no separate Herdr integration to install. The reporter supplies lifecycle state and an ID-based native session reference; Herdr resumes it with `felan --session <id>`. The ID is resolved from Felan's configured or default session directory, so a one-off `--session-dir` outside that directory cannot be restored automatically.
